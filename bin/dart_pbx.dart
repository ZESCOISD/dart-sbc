...
void main() async {
  ...
  int? wsPort =env['WS_SERVER_PORT']!=null? int.parse(env['WS_SERVER_PORT']!):null;

  String? secureTcpIp = env['SEC_TCP_SERVER_ADDRESS'];
  int? secureTcpPort =env['SEC_TCP_SERVER_PORT']!=null? int.parse(env['SEC_TCP_SERVER_PORT']!):null;
 String? path_to_certificate_file = env['PATH_TO_CERTIFICATE_FILE_PEM'];
  ...
  int? udpPort = env['UDP_SERVER_PORT']!=null?int.parse(env['UDP_SERVER_PORT']!):null;
  ...
  if(secureTcpIp!=null) {
    SecureTcpSipServer(secureTcpIp,secureTcpPort!,path_to_certificate_file!,path_to_private_key_file!);
  }
